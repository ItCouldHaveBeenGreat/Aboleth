...
def parse_monster_from_aidedd_link(session, url):
    ...
    for attribute in attributes:
        ...
        value = attribute[1].replace('<br/>', '').lower().strip()
        ...
        if key in ('saving_throws', 'skills'):
        ...
        elif key in ['damage_immunities', 'damage_resistances']:
            ...
            non_psb_types = reduce(lambda agg, add: agg + "," + add,
                                   filter(lambda type: PSB_CONSTANT not in type,
                                          value.split(';')))
            monster_data[key] = [type.strip() for type in non_psb_types.split(',')] + [type.strip() for type in psb_types]
        elif key in ['condition_immunities','languages', 'senses', 'speed']:
    ...
    for feature_html in stat_block.find('div', class_='sansSerif').find_all(['p', 'div'], recursive=False):
        ...
        if feature_html.name == 'div':
            ...
            if phase == 1:
                monster_features.append({
                    'name': feature_html.contents[0].text,
                    'description': feature_html.contents[1][2:]})
            elif phase == 2:
                ...
                if attack_attributes:
                    ...
                    # TODO: lus 18 (4d8) radiant damage. There are some attacks that have multiple damage types! Account for them
                    monster_actions[attack['name'].lower()] = attack

                # Parse multi-attacks... there's three formats I've seen so far
...
def get_urls_to_query(session):
    #return ['https://www.aidedd.org/dnd/monstres.php?vo=adult-black-dragon']
...
if __name__ == '__main__':
    ...
    #parsed_block = parse_monster_from_aidedd_link(session, 'https://www.aidedd.org/dnd/monstres.php?vo=aboleth')
    #parsed_block = parse_monster_from_aidedd_link(session, 'https://www.aidedd.org/dnd/monstres.php?vo=bearded-devil')
    #parsed_block = parse_monster_from_aidedd_link(session, 'https://www.aidedd.org/dnd/monstres.php?vo=marilith')
    monster_json = []
